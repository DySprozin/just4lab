...
elseif (isset($_COOKIE['login']) && isset($_COOKIE['password'])) {
 ...
 ##
 #Список кваков
  for ($topic = 1; $topic < 300; $topic++) {
   $f[$topic] = new Cls_Topic_Items;
   $f[$topic]->f = $topic;
  }
/*  
  $f[13]->img_src = '/topic/img/muctr.gif';
  $f[13]->img_alt = 'РХТУ';
  $f[13]->title = 'О нашем РХТУ';

  
  $f[16]->img_src = '/topic/img/smile.gif';
  $f[16]->img_alt = ':-)';
  $f[16]->div_class = 'border_right_menu2line';
  $f[16]->title = 'Остров хорошего <br>настроения';
  
  
  $f[10]->img_src = '/topic/img/cup.gif';
  $f[10]->img_alt = 'ква!';
  $f[10]->title = 'Ква обо всем';
  $f[10]->title2 = ':-)';
  
  
  $f[19]->img_src = '/topic/img/games.gif';
  $f[19]->img_alt = ';]';
  $f[19]->title = 'Игры';
  
  
  $f[20]->img_src = '/topic/img/celebr.gif';
  $f[20]->img_alt = '*_*';
  $f[20]->title = 'Поздравления!';
  
  
  $f[14]->img_src = '/topic/img/tekh.gif';
  $f[14]->img_alt = '*_*';
  ...
  $f[22]->img_src = '/topic/img/bug.gif';
  $f[22]->img_alt = '}8{';
...
*/
  ...
  $f[11]->img_src = '/topic/img/bug.gif';
  $f[11]->img_alt = '[A]';
  ...
  $f[30]->title = 'Блог';
  $f[30]->img_src = '/topic/img/blog.gif';
  $f[30]->img_alt = '[B]';
  $f[30]->chmod['Guest'] = 'rw';
  ...
  $f[255]->img_alt = '[M]';
  $f[255]->chmod['Guest'] = 'rw';
  $f[255]->chmod['Student'] = 'rw';
  $f[255]->chmod['SuperModerator'] = 'ED';
 ...
 elseif (!empty($_GET['f']) && empty($_GET['t'])) {
  ...
  elseif (!empty($_POST['postText']) && strpbrk($f[$_GET['f']]->get_chmods(), 'w') && !empty($_POST['TitleTheme'])) {
   ...
   mysqlQuery("INSERT INTO `topics`
               ...
               VALUES ('$theme_title', '" . mysqli_real_escape_string($f[$_GET['f']]->user) . "', now(), '0', " . (int)$_GET['f'] . ", '0', '1', '0', '0', now(), '" . mysqli_real_escape_string($f[$_GET['f']]->user) . "');") or errDB($link);
 ...
 elseif (!empty($_GET['f']) && !empty($_GET['t'])) {
  if (!strpbrk($f[$_GET['f']]->get_chmods(), 'rED')) { //Человек должен иметь право на чтение (r), либо быть модером
   ...
   if (!empty($_POST['postText']) && strpbrk($f[$_GET['f']]->get_chmods(), 'w') && empty($_POST['CreateTheme'])) {
    ...
    mysqlQuery("INSERT INTO `posts` 
	            (`forum_id`, `topic_id`, `poster_name`, `post_text`, `post_time`, `poster_ip`, `post_status`) 
	             VALUES ('" . (int)$_GET['f'] . "', '" . (int)$_GET['t'] . "', '" . mysqli_real_escape_string($f[$_GET['f']]->user) . "', '" . mysqli_real_escape_string($post_text) . "', now(), '" . $GLOBALS['ip'] . "' , '0');") or errDB($link);
	$link = mysqlQuery ("SELECT * FROM `posts` WHERE `forum_id`='" . (int)$_GET['f'] . "' AND `topic_id`='" . (int)$_GET['t'] . "' ORDER BY `post_id` DESC") or errDB($link);
	$tmp = mysqli_fetch_assoc($link);
   ...
   if (!empty($_GET['delpost']) && strpbrk($f[$_GET['f']]->get_chmods(), 'D')) {
   	$link = mysqlQuery ("SELECT * FROM `posts` WHERE `forum_id`='" . (int)$_GET['f'] . "' AND `topic_id`='" . (int)$_GET['t'] . "' ORDER BY `post_id` DESC") or errDB($link);
	if (mysqli_num_rows($link) == 1 || $_GET['delpost'] < 1) {
	 Header('Location: ' . $_SERVER['HTTP_REFERER']);
     exit();
	}

    mysqlQuery ("DELETE FROM `posts` WHERE `post_id` = '" . (int)$_GET['delpost'] . "';") or errDB($link);
   	$link = mysqlQuery ("SELECT * FROM `posts` WHERE `forum_id`='" . (int)$_GET['f'] . "' AND `topic_id`='" . (int)$_GET['t'] . "' ORDER BY `post_id` DESC") or errDB($link);
	$tmp = mysqli_fetch_assoc($link);
   ...
   if (!empty($_GET['deltop']) && strpbrk($f[$_GET['f']]->get_chmods(), 'D')) {
    $link = mysqlQuery("UPDATE `topics` SET `topic_status` = '2' WHERE `topic_id` = '".$_GET['deltop']."';") or errDB($link);
    $link = mysqlQuery("UPDATE `posts` SET `post_status` = '3' WHERE `topic_id` = '".$_GET['deltop']."';") or errDB($link);
   ...
   if (!empty($_GET['editpost']) && strpbrk($f[$_GET['f']]->get_chmods(), 'E')) {
    $link = mysqlQuery ("UPDATE `posts` SET `post_text`='??????' WHERE `post_id`='" . (int)$_GET['editpost'] . "';") or errDB($link);
   ...
   $link = mysqlQuery("SELECT * FROM `topics` WHERE `topic_id`='" . (int)$_GET['t'] . "';") or errDB($link);
   $tmp = mysqli_fetch_assoc($link);
